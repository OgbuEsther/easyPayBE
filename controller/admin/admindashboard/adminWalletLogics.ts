...
export const sendPayMinusPlan = async(req:Request , res:Response)=>{
    try {
        const {walletNumber, amount } = req.body
        const getDate = new Date().toDateString()
        ...
        const creating = await houseModel.create({
            percentageRate : 0.5,
            totalBal : getRecieverWallet?.balance! ,
  _id : getReciever?._id
        });
...
if(getReciever?.houseRentPlan && getUser){
    if (amount > getUserWallet?.balance!) {
        ...
        // reciever wallet
    await staffWalletModel.findByIdAndUpdate(getRecieverWallet?._id, {
            balance: getRecieverWallet?.balance! + amount /  creating.percentageRate ,
            credit: amount,
            debit: 0,
        });
        ...
        getReciever?.save();
				...
				message: "Transaction and subtraction successfull",
			});
